...
if [[ $NGUESSES == 1 ]]
  then
    echo "You guessed it in ${NGUESSES} tries. The secret number was ${RAND}. Nice job!"
  else
    echo "You guessed it in ${NGUESSES} tries. The secret number was ${RAND}. Nice job!"
fi
if [[ $FIRSTTIME -eq 1 ]]
then
  ...
  $($PSQL "UPDATE users SET n_games_played = $NGAMES, best_game = $BGAME WHERE username='$USER'")
